...
-- Operaciones admin (solo para service_role)
CREATE POLICY "admin_manage_products" ON productos
...
-- Operaciones admin (solo para service_role)
CREATE POLICY "admin_manage_clients" ON clientes
FOR ALL USING (auth.role() = 'service_role');
...
-- Operaciones admin (solo para service_role)
CREATE POLICY "admin_manage_service_orders" ON ordenes_servicio
FOR ALL USING (auth.role() = 'service_role');
