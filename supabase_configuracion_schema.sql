-- =====================================================
-- ESQUEMA PARA CONFIGURACIÓN DE LA APLICACIÓN
-- =====================================================

-- Tabla de configuración general del sitio
CREATE TABLE configuracion_sitio (
    id SERIAL PRIMARY KEY,
    seccion VARCHAR(50) NOT NULL UNIQUE, -- 'sitio', 'tema', 'analytics', etc
    configuracion JSONB NOT NULL, -- Datos de configuración en formato JSON
...
CREATE TABLE servicios_generales (
    ...
    activo BOOLEAN DEFAULT true,
    orden INTEGER DEFAULT 0, -- Para ordenar los servicios
    date_added TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    last_modified TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE servicios_precios (
    id SERIAL PRIMARY KEY,
    servicio_id VARCHAR(100) NOT NULL UNIQUE, -- ID del servicio (ej: 'limpieza-advance-cpu')
    nombre VARCHAR(200) NOT NULL,
    categoria VARCHAR(100) NOT NULL, -- 'Mantenimiento', 'Up-Grade', 'Reparaciones'
    subcategoria VARCHAR(100), -- 'Limpiezas', 'Mejoras', etc
...
CREATE INDEX idx_configuracion_seccion ON configuracion_sitio(seccion);
CREATE INDEX idx_servicios_categoria ON servicios_generales(categoria);
CREATE INDEX idx_servicios_precios_categoria ON servicios_precios(categoria);
CREATE INDEX idx_servicios_precios_activo ON servicios_precios(activo);

-- Función para actualizar last_modified automáticamente (reutilizamos la existente)
